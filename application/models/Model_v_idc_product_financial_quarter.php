...
class Model_v_idc_product_financial_quarter extends MY_Model {

	private $primary_key    = NULL;
	private $table_name 	= 'v_idc_product_financial_quarter';
	...
	private function create_json($query_data) {
		...
		foreach($query_data as $qd) {
			if(array_key_exists($qd->Product, $return_array)) {
				if(array_key_exists($qd->quarter, $return_array[$qd->Product])) {
	...
	public function count_all($q = null, $field = null)
	{
		...
		if (empty($field)) {
			foreach ($this->field_search as $field) {
				if ($iterasi == 1) {
					$where .= "v_idc_product_financial_quarter.".$field . " LIKE '%" . $q . "%' ";
				} else {
					$where .= "OR " . "v_idc_product_financial_quarter.".$field . " LIKE '%" . $q . "%' ";
				}
				$iterasi++;
			}
...
/* End of file Model_v_idc_product_financial_quarter.php */
/* Location: ./application/models/Model_v_idc_product_financial_quarter.php */
