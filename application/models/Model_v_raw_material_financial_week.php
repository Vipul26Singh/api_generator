...
class Model_v_raw_material_financial_week extends MY_Model {

	private $primary_key    = NULL;
	private $table_name 	= 'v_raw_material_financial_week';
	private $field_search 	= ['Raw_Material', 'qty', 'week'];
	...
	public function this_filter() {
		...
		foreach($this->input->get() as $key => $val) {
			...
			if($key == 'raw_material_name' && !empty(trim($val))) {
	...
	private function create_json($query_data) {
		...
		foreach($query_data as $qd) {
			if(array_key_exists($qd->Raw_Material, $return_array)) {
				$return_array[$qd->Raw_Material][$qd->week]['qty'] += floatval($qd->qty);
			} else {
	...
	public function count_all($q = null, $field = null)
	{
		...
		if (empty($field)) {
			foreach ($this->field_search as $field) {
				if ($iterasi == 1) {
					$where .= "v_raw_material_financial_week.".$field . " LIKE '%" . $q . "%' ";
				} else {
					$where .= "OR " . "v_raw_material_financial_week.".$field . " LIKE '%" . $q . "%' ";
				}
				$iterasi++;
			}
...
/* End of file Model_v_raw_material_financial_week.php */
/* Location: ./application/models/Model_v_raw_material_financial_week.php */
